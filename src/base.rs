...
fn increment_with_bases(input: Vec<u8>, bases: Vec<usize>) -> Vec<u8> {
    let mut number = from_digits_and_bases(input, bases.clone());

    let mut result = to_digits_and_bases(number + 1, bases.clone());
    while result.clone().into_iter().any(|a| a == 0) {
        number = number + 1;
        result = to_digits_and_bases(number + 1, bases.clone());
    }
...
fn from_digits_and_bases(input: Vec<u8>, bases: Vec<usize>) -> u64 {
    let mut n = 0;
    let base_len: isize = bases.len() as isize;
    input.into_iter().enumerate().for_each(|(i, d)| {
        n = n * bases[((base_len - i as isize - 1) % base_len) as usize] as u64 + d as u64
    });
    n
}
...
fn to_digits_and_bases(mut input: u64, bases: Vec<usize>) -> Vec<u8> {
    ...
    let mut i: usize = 0;
    while input > 0 {
        result.insert(0, (input % bases[((base_len - i as isize - 1) % base_len) as usize] as u64) as u8);
        input = input / bases[((base_len - i as isize - 1) % base_len) as usize] as u64;
        i = (i + 1) % bases.len();
    }
    result
}
...
mod tests {
    ...
    fn it_works_2() {
        assert_eq!(
            super::increment_with_bases(vec![23, 24, 25], vec![26]),
            vec![23, 25, 1]
        );
        assert_eq!(
            super::increment_with_bases(vec![23, 25, 25], vec![26]),
            vec![24, 1, 1]
        );
    }
