...
pub fn run_gpu(
    ...
    let lhs_data: Vec<u32> = (0..256).collect();
    let rhs_data: Vec<u32> = (0..256).map(|i| i * 2).collect();
    ...
    let rhs_buffer = Buffer::from_iter(
        memory_allocator.clone(),
        BufferCreateInfo {
            usage: BufferUsage::STORAGE_BUFFER,
            ..Default::default()
    ...
    let mismatches = results
        ...
        .enumerate()
        .filter_map(|(idx, (sum, (lhs, rhs)))| {
            if *sum != lhs + rhs {
                Some((idx, *sum, lhs + rhs))
            } else {
                None
            }
        })
    ...
    if mismatches.is_empty() {
        println!("Addition verified for {} elements", results.len());
    } else {
